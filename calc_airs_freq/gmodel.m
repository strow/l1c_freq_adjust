...
% d = size(yoff);
% if (length(d) ~= 2 )
%    error('argument yoff must be dimensioned [17 x 1]')
% end

% ymod from get_ymod is not in the detector module ordering that gmodel uses.  
% so flip here NOT in calling program
fc = 1:17;
gm = fliplr(fc);
yoff = yoff(gm);
